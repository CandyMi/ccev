...
TEST(dns_set_server_valid) {
    ...
    ASSERT(ccev_dns_set_server(loop, "1.1.1.1", 53) == CCEV_OK);
    ccev_loop_destroy(loop);
}

TEST(dns_set_server_invalid) {
    ccev_loop_t *loop = ccev_loop_create(64);
    ASSERT(ccev_dns_set_server(NULL, "1.1.1.1", 53) == CCEV_ERR);
    ASSERT(ccev_dns_set_server(loop, "not-an-ip", 53) == CCEV_ERR);
    ccev_loop_destroy(loop);
}
...
TEST(dns_set_server_replaces) {
    ...
    ASSERT(ccev_dns_set_server(loop, "8.8.8.8", 53) == CCEV_OK);
    ccev_loop_destroy(loop);
}
